feature 'Create Activity' do
  ...
  context 'a business exists' do
    ...
    context 'an activity does not exist' do
      ...
      context 'start_time is omitted' do
        scenario 'user sees start_time is required' do
          complete_activity_form(start_time: '')
          expect(page).to have_content "can't be blank"
        end
      end

      context 'end_time is omitted' do
        scenario 'user sees end_time is required' do
          complete_activity_form(end_time: '')
          expect(page).to have_content "can't be blank"
        end
      end
      ...
      context 'wrong end_time scheduling' do
        context 'end_time is scheduled before start_time' do
          scenario 'user sees end_time need to schedule after start_time' do
            ...
            expect(page).to have_content "can't be scheduled before or equal start_time"
          end
        end
  ...
  def complete_activity_form(overrides={})
    within '#new_activity' do
      ...
      fill_in 'activity_start_time', with: overrides[:start_time] || '08:00:00'
      fill_in 'activity_end_time', with: overrides[:end_time] || '16:00:00'
      click_button 'Submit'
    end
  end
end
